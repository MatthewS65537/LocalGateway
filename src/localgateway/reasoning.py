...
def _wants_normalization(provider: ProviderConfig) -> bool:
    return (provider.reasoning_mode or "auto") == "auto"
...
def normalize_response_body(provider: ProviderConfig, body: bytes) -> bytes:
    ...
    if not _wants_normalization(provider):
        return body
...
def transform_stream_chunk(provider: ProviderConfig, chunk: bytes, parser: SSEParser) -> bytes:
    ...
    In passthrough mode, bytes are returned untouched. In auto mode, chunks are
    buffered to event boundaries (via the caller-owned parser) and only
    re-serialized when a reasoning field was actually rewritten; other events
